...
<div class="position-relative plant-card" style="width:<?= $width ?>px; height:250px; background: linear-gradient(180deg, rgba(2, 87, 5, 0.248) 0%, rgba(255, 255, 255, 0.341) 100%), url(image/<?= $_POST['gambar'] ?>); <?php if (!$mini) {
        ...
        <div style="display:flex; flex-direction: column; margin-bottom:20px;">
            ...
            <div>
                ...
                <p style="text-shadow: 0px 30px 30px #000000; <?php if ($mini) {
                                                                ...
                                                                } ?>" id="manager-content"><?= $_POST['pengelola'] ?></p>
            </div>
            <div>
                <p style="display:<?php if ($mini) {
                                        echo 'none';
                                    } else {
                                        echo 'block';
                                    } ?>; text-shadow: 0px 30px 30px #000000; font: size 14px; "><?= substr($_POST['des'], 0, 100) ?></p>
            </div>
            <div style="margin-top:10px">
                <button type="button" class="btn btn-success" style="border-radius: 50px; font-size: <?php if ($mini) {
                                                                                                            echo '10px; padding:2px 4px;';
                                                                                                        } else {
                                                                                                            echo '12px';
                                                                                                        } ?>; line-height:14px; font-weight: bold; display: inline-block; margin-right:10px; color:white">
                    Adopt Now <i class='fas fa-dollar-sign'></i>
                </button>
                <p style="font-size: <?php if ($mini) {
                                            echo '14px';
                                        } else {
                                            echo '18px; display:inline-block;';
                                        } ?>">
                    IDR <?= $_POST['harga'] ?>
